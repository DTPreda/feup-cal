...
Result nearestPoints_BF(std::vector<Point> &vp) {
    ...
    for(int i = 0; i < vp.size(); i++){
        for(int j = i + 1; j < vp.size(); j++){
            if(i != j) if(vp[i].distance(vp[j]) < res.dmin){
                res.p1 = vp[i];
                res.p2 = vp[j];
...
Result minResult(Result first, Result second){
    if(first.dmin < second.dmin){
        return first;
    }
    return second;
}
...
Result nearestPoints_DC(std::vector<Point>& vp) {
    ...
    if (vp.size() == 2) {
        Result res(vp.at(0).distance(vp.at(1)),vp.at(0), vp.at(1));
        return res;
    }
    std::vector<Point> left_pairs, right_pairs;
    double half = (vp.at(0).x + vp.at(vp.size() - 1).x)/2;
    for (int i = 0; i < vp.size() / 2; ++i) {
        left_pairs.push_back(vp.at(i));
        right_pairs.push_back(vp.at(vp.size() / 2 + i));
    }
    Result left_smallest = nearestPoints_DC(left_pairs);
    Result right_smallest = nearestPoints_DC(right_pairs);
    Result middle_smallest;
    Result both_smallest = minResult(left_smallest, right_smallest);
    bool found = false;
    double delta = both_smallest.dmin;
    std::vector<Point> groomed;
    for (auto & i : vp) {
        if (i.x < (half - delta) || i.x > (half + delta))
            continue;
        else
            groomed.push_back(i);
    }
    std::sort(groomed.begin(), groomed.end(), [](Point p, Point q){ return p.y < q.y || (p.y == q.y && p.x < q.x);});
    for (int i = 0; i < vp.size(); ++i) {
        for (int j = i + 1; j < vp.size(); ++j) {
            if (std::abs(vp.at(i).y - vp.at(j).y) > delta) {
                break;
            }
            else if (vp.at(i).distance(vp.at(j)) < delta) {
                found = true;
                delta = vp.at(i).distance(vp.at(j));
                middle_smallest = Result(delta, vp.at(i), vp.at(j));
            }
        }
    }
    if (!found)
        return both_smallest;
    else
        return middle_smallest;
}

...
bool changeMakingDP(unsigned int C[], unsigned int Stock[], unsigned int n, unsigned int T, unsigned int usedCoins[]) {
    ...
    std::vector<int> bestCoin(T+1, 0);

    for(int i = 0; i < T + 1; i++) {
        for(int j = 0; j < n; j++){
            if(C[j] <= i) bestCoin[i] = j;
        }
    }

    while (T != 0) {
        for(int i = 0; i < n; i++) {
            if(C[i] == C[bestCoin[T]] && Stock[i] > 0){
                usedCoins[i]++;
                Stock[i]--;
                T -= C[bestCoin[T]];
                break;
            } else if (C[i] == C[bestCoin[T]] && Stock[i] == 0) {
                if(i == 0) return false;
                for(int j = 0; j < T + 1; j++){
                    if(bestCoin[j] == bestCoin[T]) bestCoin[j] -= 1;
                }
            }
        }
    }
